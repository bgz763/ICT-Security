...
PGP also employs public-key cryptography for key exchange. Each user has a pair of public and private keys. The public key is used for encryption, and the private key is used for decryption. Users share their public keys openly, but keep their private keys secret.
...
##### Digital Signatures:
PGP uses digital signatures to ensure the integrity of the message. Before sending a message, the sender signs it with their private key. The recipient can verify the signature using the sender's public key. If the message has been tampered with, the signature will fail to verify.
...
The --encrypt flag is used to encrypt a message. The recipient's public key is specified in the encryption process.

The --sign flag is used to sign a message, and the sender's private key is used for signing.

The --armor flag is often used to generate ASCII-armored output, which is a textual representation of the encrypted or signed data. This is useful for including PGP-protected messages in email or other text-based communication.

The --recipient flag specifies the recipient's key when encrypting a message.

The --clearsign flag is used for clear-signing a message, which involves signing the message in a way that the original message remains readable.
